...
function date_get_timestamp($d) { return $d->format('U'); }
...
function preg_match_1($p, $s) {   if (js()) { return $s.match(new RegExp(trim_slashes($p), "i")); } else { $m = array(); if (preg_match($p, $s, $m) !== FALSE) { return $m[0]; } else { return null; } } }
...
function preg_matches($p, $s) {   if (js()) { return $s.match(new RegExp(trim_slashes($p), "gi")); } else { $m = array(); if (preg_match_all($p, $s, $m, PREG_PATTERN_ORDER) !== FALSE) { return $m[0]; } else { return null; } } }
...
function date_get_rfc3339($d) { if (js()) { return strcat($d.getFullYear(), '-', str_pad_left(1+$d.getUTCMonth(), 2, "0"), '-', str_pad_left($d.getDate(), 2, "0"), 'T', str_pad_left($d.getUTCHours(), 2, "0"), ':', str_pad_left($d.getUTCMinutes(), 2, "0"), ':', str_pad_left($d.getUTCSeconds(), 2, "0"), 'Z'); } else { return date_format($d, 'c'); } }
...
function date_get_ordinal_days($d) { if (js()) { return ymdp_to_d($d.getFullYear(), 1+$d.getMonth(), $d.getDate()); } else { return 1+date_format($d, 'z'); } }
...
function xp_has_rel($s) { return strcat("//*[contains(concat(' ',@rel,' '),' ", $s, " ')]"); }
function xpr_has_rel($s) { return strcat(".//*[contains(concat(' ',@rel,' '),' ", $s, " ')]"); }
function xpr_attr_starts_with_has_rel($a, $s, $r) { return strcat(".//*[contains(concat(' ',@rel,' '),' ", $r, " ') and starts-with(@", $a, ",'", $s, "')]"); }
...
function tw_text_proxy() {   $isjs = js(); $args = $isjs ? arguments : func_get_args(); if (count($args) === 0) { return ''; } $t = $args[0]; $re = auto_link_re(); $ms = preg_matches($re, $t); if (!$ms) { return $t; } $mlen = count($ms); $sp = preg_split($re, $t); $t = ""; $sp[0] = string($sp[0]); for ($i=0; $i<$mlen; $i++) { $mi = $ms[$i]; $spliti = $sp[$i]; $t = strcat($t, $spliti); $sp[$i+1] = string($sp[$i+1]); if (substr($sp[$i+1], 0, 1)=='/') { $sp[$i+1] = substr($sp[$i+1], 1, strlen($sp[$i+1])-1); $mi = strcat($mi, '/'); } $spe = substr($spliti, -2, 2); if ($mi[0]!='@' && (substr($mi, -3, 1) !== '.' || substr_count($mi, '.') > 1)) { $afterlink = ''; $afterchar = substr($mi, -1, 1); while (contains('.!?,;"\')]}', $afterchar) && ($afterchar !== ')' || !contains($mi, '('))) { $afterlink = strcat($afterchar, $afterlink); $mi = substr($mi, 0, -1); $afterchar = substr($mi, -1, 1); } $prot = protocol_of_uri($mi); $proxy_url = ''; if ($prot === 'irc:') { $proxy_url = $mi; } else { $proxy_url = 'https://j.mp/0011235813'; } $t = strcat($t, $proxy_url, $afterlink); } else { $t = strcat($t, $mi); } } return strcat($t, $sp[$mlen]); }
...
function tw_url_to_status_id($u) { if (!$u) return 0; $u = explode("/", string($u)); if ($u[2] != "twitter.com" || $u[4] != "status" || !ctype_digit($u[5])) { return 0; } return $u[5]; }
